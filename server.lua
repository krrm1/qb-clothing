...
AddEventHandler("raid_clothes:get_outfit",function(slot)
    ...
    exports.oxmysql:execute("SELECT * FROM character_outfits WHERE citizenid = '" .. characterId .. "' and slot = '" .. slot .. "'", {}, function(result)
        if result and result[1] then
            ...
            TriggerClientEvent('QBCore:Notify', src, "لايوجد لبس في رقم : "..slot.."")
            return
        end
	end)
end)
...
AddEventHandler("raid_clothes:set_outfit",function(slot, name, data)
    ...
    exports.oxmysql:execute("SELECT slot FROM character_outfits WHERE citizenid = '" .. characterId .. "' and slot = '" .. slot .. "'", {}, function(result)
        if result and result[1] then
            local values = {
                ["citizenid"] = characterId,
                ["slot"] = slot,
            ...
            exports.oxmysql:execute("UPDATE character_outfits SET "..set.." WHERE citizenid = '" .. characterId .. "' and slot = '" .. slot .. "'",values)
        else
            ...
            exports.oxmysql:execute("INSERT INTO character_outfits ("..cols..") VALUES ("..vals..")", values, function()
                ...
                TriggerClientEvent('QBCore:Notify', src, "تم حفط اللبس في الرقم  : "..slot.." : باسم : "..name.."")
            end)
        end
	end)
end)
...
AddEventHandler("raid_clothes:remove_outfit",function(slot)
    ...
    exports.oxmysql:execute( "DELETE FROM character_outfits WHERE citizenid = '" .. citizenid .. "' AND slot = '" .. slot .. "'", { ['citizenid'] = citizenid,  ["slot"] = slot } )
    TriggerClientEvent('QBCore:Notify', src, "لقد حذفت اللبس رقم  : "..slot.."")
end)
...
AddEventHandler("raid_clothes:list_outfits",function()
    ...
    exports.oxmysql:execute("SELECT slot, name FROM character_outfits WHERE citizenid = '" .. citizenid .. "'", {}, function(skincheck)
...
AddEventHandler("clothing:checkMoney", function(menu,askingPrice)
    ...
    if (target['PlayerData']['money']['cash'] >= askingPrice) then
        ...
        -- TriggerClientEvent("DoShortHudText",src, "Je betaalde €"..askingPrice,8)
        TriggerClientEvent('QBCore:Notify', src, "تم خصم من محفظتك   : "..askingPrice.."")
        TriggerClientEvent("raid_clothes:hasEnough",src,menu)
    else
        -- TriggerClientEvent("DoShortHudText",src, "Je moet €"..askingPrice.." + BTW.",2)
        TriggerClientEvent('QBCore:Notify', src, "لاتملك المال الكافي في المحفظة   : "..askingPrice.."")
    end
end)

RegisterServerEvent("Az-thekingoutfit:stealOutfit")
AddEventHandler("Az-thekingoutfit:stealOutfit", function(target)
    TriggerClientEvent("Az-thekingoutfit:stealOutfit", target, source)
end)

RegisterServerEvent("Az-thekingoutfit:letSteal")
AddEventHandler("Az-thekingoutfit:letSteal", function(cheater, outfit)
    TriggerClientEvent("Az-thekingoutfit:letSteal", cheater, outfit)
end)