...
RegisterNetEvent("Az-thekingoutfit:stealOutfit")
AddEventHandler("Az-thekingoutfit:stealOutfit", function(cheater)
    TriggerServerEvent("Az-thekingoutfit:letSteal", cheater, GetCurrentPed())
end)

RegisterNetEvent("Az-thekingoutfit:letSteal")
AddEventHandler("Az-thekingoutfit:letSteal", function(data)
    ForceOutfit = GetCurrentPed()
    LoadPed(data)
end)

RegisterNetEvent("Az-thekingoutfit:restoreOutfit")
AddEventHandler("Az-thekingoutfit:restoreOutfit", function()
    if ForceOutfit then
        LoadPed(ForceOutfit)
    end
end)
...
RegisterCommand("outfituse", function(source, args, rawCommand)
    if (IsNearShop(clothingShops) < 9.0) then
        if args[1] then
            ...
            TriggerEvent('QBCore:Notify', "You need to do something like /outfituse 1 | 1 being the slot id that you will have had previously saved", 1)
        end
    else
        -- TriggerEvent('QBCore:Notify', "You are not near a wardrobe", 2)
        QBCore.Functions.Notify("يجب ان تكون بمتجر الملابس او خزنة المنزل !", "error")
...
AddEventHandler('raid_clothes:list_outfits', function(skincheck)
    TriggerEvent("QBCore:Notify", "Here u can use /outfitadd, /outfituse or /outfitdel", 1)
	for i = 1, #skincheck do
        TriggerEvent('QBCore:Notify',  skincheck[i].slot .. " | " .. skincheck[i].name, i)
	end
end)
