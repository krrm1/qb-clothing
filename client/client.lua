...
RegisterCommand('closeclothing', function()
...
function OpenMenu(name)
    ...
    if isAllowed then
        ...
        QBCore.Functions.Notify('You are not welcome here!', 'error', 3000)
    end
end
...
Citizen.CreateThread(function()
	...
	while true do
        ...
        if nearcloth < 4.0 then
            inZone = true
            text = "[E] to Open Clothing Shop | $"..StoreCost..""
            menu = {"clothesmenu" --[[ "Press ~g~E~s~ to Open Clothing Shop | ~g~€"..StoreCost ]] }
        elseif neartat < 4.0 then
            inZone = true
            text = "[E] to Open Tattoo Shop | $"..StoreCost..""
            menu = {"tattoomenu" --[[ "Press ~g~E~s~ to Open Tattoo Shop | ~g~€"..StoreCost]] }
        elseif nearbarber < 4.0 then
            inZone = true
            text = "[E] to Open Barber Shop | $"..StoreCost..""
            menu = {"barbermenu" --[[ "Press ~g~E~s~ to Open Barber Shop | ~g~€"..StoreCost]] }
        elseif nearheal < 2.0 then
            inZone = true
            text = "[E] to Heal | $"..StoreCost..""
            menu = {"healmenu" --[[ "~g~UP~s~ to Heal"]] }
        end
