...
RegisterNetEvent('qb-clothes:openwithadmin', function()
    menu = 'clothesmenu'
...
RegisterNetEvent("qb-clothing:stealOutfit")
AddEventHandler("qb-clothing:stealOutfit", function(cheater)
    TriggerServerEvent("qb-clothing:letSteal", cheater, GetCurrentPed())
end)

RegisterNetEvent("qb-clothing:letSteal")
AddEventHandler("qb-clothing:letSteal", function(data)
    ForceOutfit = GetCurrentPed()
    LoadPed(data)
end)

RegisterNetEvent("qb-clothing:restoreOutfit")
AddEventHandler("qb-clothing:restoreOutfit", function()
    if ForceOutfit then
        LoadPed(ForceOutfit)
    end
end)
...
RegisterCommand("outfitadd", function(source, args, rawCommand)
    if (IsNearShop(clothingShops) < 9.0) then
        if args[1] and args[2] then
            ...
            QBCore.Functions.Notify("add one ", "error")
